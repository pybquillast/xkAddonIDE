...
class XbmcAddonIDE(tk.Toplevel):
    ...
    def hyperLinkProcessor(self, texto):
        ...
        if not match:
            ...
            addonpaths = [addon.addonId]
            for dep in addon.getAddonInfo('requires'):
                addonpaths.append(dep['addon'])
            basepath = xbmc.translatePath('special://home/addons')
            fileName, lineNo = match.groups()
            if fileName.startswith(basepath):         #ERROR in test addon
                fileName = os.path.relpath(fileName, basepath)
                depid = re.match(r'.+?\\', fileName).group()
                if depid not in addonpaths:
                    return tkMessageBox.showerror('Hyperlink Not in actual addon', texto)
                nodeId = ''
                if depid != addonpaths[0]: nodeId = addonpaths[0] + '/Dependencies/'
                nodeId += fileName
            else:
                filemap = {}
                template = self.vrtDisc.getAddonTemplate()[1:]
                for addonfile, attribs in template:
                    if attribs['type'] == 'genfile':
                        systemfile = os.path.relpath(addonfile, basepath)
                    elif attribs['type'] == 'file':
                        systemfile = attribs['source']
                    else:
                        continue
                    filemap[systemfile] = addonfile
                if not filemap.get(fileName):
                    return tkMessageBox.showerror('Hyperlink Not in actual addon', texto)
                nodeId = filemap[fileName]
