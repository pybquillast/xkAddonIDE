...
class vrtDisk:
    ...
    def getRequiredDirectories(self):
        ...
        for key, dirs in reqDirectories.items():
            ...
            if bFlag:
                reqdirs.update(dirs)
        directories = []
        dirtypes = {'Dependencies':'requires', 'datadir':'repdatadir'}
    ...
    def sincronizeRepoTo(self, ftp, datadir='/public_html/'):
        addonSettings = self._addonSettings
        ...
        if not isinstance(checksum_dir, basestring):
            repo_checksum = ''
        ...
        if not isinstance(repo_info, basestring):
            repo_info = []
        else:
        ...
        while relativedir:
            ...
            try:
                ...
                moddir.append(dirname)
                ftp.mkd(dirname)

        ftp.cwd(dirname)
        for item in toUpdate.union(toDelete):
            files = filter(lambda x: x not in ['.', '..'], ftp.nlst(item))
            map(lambda x: ftp.rename(item + '/' + x, item + '/' + x + '.bak'), files)
            map(lambda x: oldVersion.add(item + '/' + x), files)
        ...
        for elem in addonFiles:
            ...
            try:
                ...
                if basedir not in datadirLst:
                    ...
                    moddir.append(dirname + '/' + basedir)
                filepath = basedir + '/' + filename
                ftp.storbinary('STOR %s' % filepath, fp)
                newVersion.add(filepath)
            except:
                bSuccess = False
                errMsg = 'While updating the repository addons an error has ocurred.\n ' \
        ...
        if bSuccess:
            ...
            try:
                for itemdir, itemname, itemcontent in repo_pairs:
                    itempath = '/' + datadir.strip('/') + '/' + urlparse.urlparse(itemdir).path.lstrip('/')
                    if itemname in datadirLst:
                        ftp.rename(itempath, itempath + '.bak')
                        oldVersion.add(itempath)
                    fp = StringIO.StringIO(itemcontent)
                    ftp.storbinary('STOR %s' % itempath, fp)
                    newVersion.add(itempath)
            except:
                errMsg = 'While updating the repository info/checksum an error has ocurred.\n ' \
                         'Rolling back the changes'

                bSuccess = False

        if not bSuccess:
            tkMessageBox.showerror('Sincronize Repo', errMsg)
            map(lambda x: ftp.delete(x), newVersion)
            map(lambda x: ftp.rename(x + '.bak', x), oldVersion)
            map(lambda x: ftp.rmd(x), reversed(moddir))
        else:
            map(lambda x: ftp.delete(x + '.bak'), oldVersion)
            map(lambda x: ftp.rmd(x), toDelete)
            tkMessageBox.showinfo('Sincronize Repo', 'Repository successfully updated')
        return
